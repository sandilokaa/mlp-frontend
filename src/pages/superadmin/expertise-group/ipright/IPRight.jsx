...
const ExpertiseGroupIPRight = () => {
    ...
    const getIPRightData = async () => {
        ...
        try {
            ...
            const getDataRequest = await axios.get(
                `http://localhost:8080/api/v1/superadmin/${superadmin.id}/ipright`,
                {
                    headers: {
                        Authorization: `Bearer ${token}`,
    ...
    const exportIPRightData = async () => {
        ...
        if (iprightData) {
            ...
            iprightData.map((item, index) => {
                return tableData.push([index + 1, item.iprightTitle, item.Lecturer.name, item.Lecturer.email]);
            });
            ...
            doc.save('data-haki.pdf', { autoDownload: false });

        }
    };

    /* ================ End Export IPRight Data ================ */
            ...
            <div id="research-content">
                <Container fluid style={{ padding: '0 32px' }}>
                    <div className="add-research-content">
                        <Row>
                            <Col xl={12}>
                                <h1 style={{ fontSize: '16px', fontWeight: '700' }}>Daftar HAKI</h1>
                            </Col>
                        </Row>
                        <Row>
                                                ...
                                                <Col xl={1} className="text-center" style={{ marginLeft: '1px' }}>
                                                    <Row style={{ display: 'flex', padding: '0', margin: '0' }}>
                                                        <Col xl={12} className="d-flex justify-content-center p-0">
                                                            <span className="view" onClick={() => navigate(`/expertisegroup/ipright/detail/${ipright.id}`)}>
                                                                <Image src={ViewIcon} />
                                                            </span>
                                                        </Col>
                                                    </Row>
...
export default ExpertiseGroupIPRight;