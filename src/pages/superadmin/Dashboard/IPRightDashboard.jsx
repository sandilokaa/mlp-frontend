...
const IPRightDashboard = () => {
    ...
    const getIPRightData = async () => {
        ...
        try {
            ...
            const getDataRequest = await axios.get(
                `http://localhost:8080/api/v1/superadmin/dashboard/iprights`,
                {
                    headers: {
            ...
            const getDataResponse = await getDataRequest.data.data.getIPRight.counts;
    ...
    const data = {
        ...
        datasets: [
            {
                label: 'Jumlah',
                data: iprightData.map(item => item.total),
                backgroundColor: [
                    '#2181E8',
                    '#D62C35',
                    '#24A560',
...
export default IPRightDashboard;